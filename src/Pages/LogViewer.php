...
class LogViewer extends SubPage implements Actions {
    ...
    public function register_sections() {
        $this->add_section( new \Qck\FeedEngine\Core\Options\DebugOptions() );
    ...
    public function content_top() {
        ...
        $html = <<<HTML
            <div class="{$prefix}-log-viewer">
                <h3>System Logs</h3>
                <textarea id="{$prefix}-log-content" readonly style="width:100%; height:300px; font-family:monospace;">{$logs}</textarea>
                <div style="margin-top: 10px;">
                    <button type="button" id="{$prefix}-refresh-logs" class="button button-link">Refresh Log File</button>
                    <button type="button" id="{$prefix}-clear-logs" class="button button-link-delete">Clear Log File</button>
                </div>
            </div>

            <script>
                jQuery('#{$prefix}-refresh-logs').on('click', function() {
                    wp.apiFetch({
                        path: {$get_path},
                        method: 'GET'
                    }).then( (response) => {
                        ...
                        if(response.success) {
                            jQuery('#{$prefix}_notices').html( response.message ).hide().fadeIn();
                            jQuery('#{$prefix}-log-content').text(response.logs);
                            jQuery(document).trigger('wp-updates-notice-added');
                        }
                        
                    } );
                });
                jQuery('#{$prefix}-clear-logs').on('click', function() {
                    ...
                    wp.apiFetch({
                        ...
                        if(res.success) {
                            wp.apiFetch({
                                ...
                                console.log(response);
                                if(response.success) {
                                    jQuery('#{$prefix}_notices').html( res.message ).hide().fadeIn();
                                    jQuery('#{$prefix}-log-content').text(response.logs);
                                    jQuery(document).trigger('wp-updates-notice-added');
                                }
