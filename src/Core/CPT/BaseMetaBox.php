...
abstract class BaseMetaBox {
    ...
    public function save_data($post_id) {
        ...
        if (!isset($_POST['_' . Manifest::PREFIX . '_' . $this->get_name() . '_nonce'])) {
            \Qck\FeedEngine\Core\Debug::logDump( 'nonce missing', __METHOD__);
            return;
        }
        if (!wp_verify_nonce($_POST['_' . Manifest::PREFIX . '_' . $this->get_name() . '_nonce'], '_' . Manifest::PREFIX . '_' . $this->get_name() . '_action')) {
            \Qck\FeedEngine\Core\Debug::logDump( 'nonce unverified', __METHOD__);
            return;
        }
    ...
    public function set($data, $name, $setValue) {
        $entry = $this->get_field_definition( $name );
        // \Qck\FeedEngine\Core\Debug::logDump( $name, __METHOD__ . ' $name');
    ...
    public function get_value_for_entry($post_id, OptionEntry $entry) {
        ...
        if( !empty($all_data) ) {
            ...
            $current = $all_data;
            foreach ($entry->path as $step) {
                // \Qck\FeedEngine\Core\Debug::logDump( $entry->path, __METHOD__ . ' $entry->path');
                if (isset($current[$step]) && is_array($current[$step])) {
                    $current = $current[$step];
                } else {
                    // \Qck\FeedEngine\Core\Debug::logDump( $current, __METHOD__ . ' $current');
                    // \Qck\FeedEngine\Core\Debug::logDump( $step, __METHOD__ . ' $step');
                    $updated_data = $this->set($all_data, $entry->key, $entry->default);
                    // \Qck\FeedEngine\Core\Debug::logDump( $entry->default, __METHOD__ . ' $entry->default');
                    // \Qck\FeedEngine\Core\Debug::logDump( $updated_data, __METHOD__ . ' $updated_data');
                    update_post_meta($post_id, '_' . Manifest::PREFIX . '_' . $this->get_name(), $updated_data);
                    // \Qck\FeedEngine\Core\Debug::logDump( $this->get($updated_data , $entry->key), __METHOD__ . ' $this->get($updated_data , $entry->key)');
                    return $entry->default; // Path broken, return default
                }
            }
            return $current[$entry->key] ?? $entry->default;
            ...
            // \Qck\FeedEngine\Core\Debug::logDump( $this->get_defaults(), __METHOD__ . 'set_defaults');
